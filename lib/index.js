...
module.exports = function(options) {
  ...
  // Legacy patterns
  seneca.add(
    { role: 'transport', hook: 'listen', type: 'queue' },
    hook_listen_beanstalk
  )
  seneca.add(
    { role: 'transport', hook: 'client', type: 'queue' },
    hook_client_beanstalk
  )
  ...
  function hook_listen_beanstalk(args, done) {
    ...
    tu.listen_topics(seneca, args, listen_options, function(topic) {
      ...
      beanstalk_out.on('connect', function() {
      ...
      beanstalk_in.on('connect', function() {
            ...
            beanstalk_in.reserve(function(err, jobid, payload) {
              ...
              if (data) {
                tu.handle_request(seneca, data, listen_options, function(out) {
                  if (out == null) return process.nextTick(do_reserve)
  ...
  function make_fivebeans(opts, note) {
    var client = new fivebeans.client(opts.host, opts.port) // eslint-disable-line new-cap
    client.id$ = nid()
    client.note$ = note
    return client
  }

  function connect_fivebeans(
    seneca,
    instance,
    position,
    direction,
    ...
    instance
      ...
      .on('close', function() {
        seneca.log.error(position, 'close', direction, topic, opts, seneca)
      })
      .connect()
